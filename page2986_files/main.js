...
function initUploadify( $fileElem, options ) {
    ...
    var url = options.url ? options.url : '/fileservice/widget/upload';
    var finalOptions = {
        auto: true,
...
// 	ajaxCall( "/pl/stat/counter/end", {
// 		uniqId : encodeURIComponent(window.gcUniqId)
// 	}, {}, function( response ) {
//
// 	} );
// });
...
function addGlobalCheckbox( element ) {
    ...
    if (window.globalCheckboxEnabled ) {
        var $checkboxEl = $('<div class="global-confirm-checkbox-block"><label><input class="global-confirm-checkbox" type="checkbox" checked name="globalConfirmCheckbox" > <span class="checkbox-text">' + window.globalCheckboxText + '</span></label></div>')
        var $checkboxInput = $checkboxEl.find('.global-confirm-checkbox')
    ...
    return function( params ) {
        if ( window.globalCheckboxEnabled && ! $checkboxInput.prop('checked') ) {
            ...
            alert("Чтобы отправить форму вы должны согласиться с условиями на обработку персональных данных. Отметьте соответствующую галочку")
            return false;
        }
        return true;
    };
